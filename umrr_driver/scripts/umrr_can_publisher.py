...
class umrr_publisher:
    ...
    def __setup_sensor(self):
        ...
        """
        print("------Antena mode-----") 
        self.parameter.sendRawParameter(2010,0, 2, msgType="w")
        
        print("------Sweep frequency mode-----") 
        self.parameter.sendRawParameter(2010, 2, 2, msgType="w")
        ...
        if rospy.has_param("~frequency_sweep"):
            
            freq_sweep = rospy.get_param("~frequency_sweep")
            self.parameter.sendRawParameter(freq_sweep[2010], freq_sweep["parameter"],
                                                       freq_sweep[2], msgType="w")
    ...
    def __get_used_atenna(self):
        ...
        self.parameter.sendRawParameter(2010, 2, msgType="r")
        res = self.__get_resp_value()
        res = res["instructionsList"][0]['value']
        print("RES::: ",res)
        rospy.loginfo(''.join("Sweep frequency: " + str(res)))
        #return '{}'.format(res)
...
if __name__ == '__main__':
    ...
    try:
        ...
        umrr_driver.__setup_sensor()
        print("get_used_atenna   ",umrr_driver.__get_used_atenna())
        
        #print(umrr_publisher.__get_used_atenna(umrr_driver))
